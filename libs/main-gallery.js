...
// ✅ main.html already has importmap for "three", so import from "three":
...
export function initMainGallery({ mountEl }) {
  ...
  function applyStyle(mesh, absAngle, rowKind, isFocused) {
    ...
    if (rowKind === "bot") { baseOpacity = 0.82; baseScale = 0.92; }
    if (rowKind === "top") { baseOpacity = 0.65; baseScale = 0.88; }
  ...
  function nearestSnapTarget() {
    ...
    for (let i = 0; i < COUNT; i++) {
      ...
      const aa = Math.abs(wrapPi(theta));
      if (aa < bestAbs) { bestAbs = aa; bestI = i; }
    }
  ...
  function tick(now) {
    ...
    camX = lerp(camX, mx * 10, 1 - Math.pow(0.94, dt * 60));
    camY = lerp(camY, my * 6, 1 - Math.pow(0.94, dt * 60));
